<div >

    <div class="space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                <h1 class="text-2xl font-bold">Services</h1>
                <p class="text-sm text-gray-500">Manage all service offerings</p>
            </div>

            <button wire:click="create" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl
                   bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700">
                <i class="fa-solid fa-plus"></i>
                Add Service
            </button>
        </div>

        <div class="bg-white dark:bg-gray-800 rounded-md shadow overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-300">
                    <tr>
                        <th class="px-5 py-3 text-left">Service</th>
                        <th class="px-5 py-3 text-left">image</th>
                        <th class="px-5 py-3 text-left">Requirements</th>
                        <th class="px-5 py-3 text-right">Actions</th>
                    </tr>
                </thead>

                <tbody class="divide-y dark:divide-gray-700">
                    @forelse($services as $service)
                        <tr>
                            <td class="px-5 py-4">
                                <p class="font-medium">{{ $service->name }}</p>
                                <p class="text-xs text-gray-500">{{ $service->slug }}</p>
                            </td>
                            <td class="px-5 py-4 flex items-center gap-3">
                                @if($service->image)
                                    <img src="{{ $service->image_url }}"
                                        class="w-10 h-10 rounded-md object-cover">
                                @endif

                                <div>
                                    <p class="font-medium">{{ $service->name }}</p>
                                    <p class="text-xs text-gray-500">{{ $service->slug }}</p>
                                </div>
                            </td>

                            <td class="px-5 py-4">
                                <div class="flex flex-wrap gap-2">
                                    @foreach($service->requirements ?? [] as $req)
                                        <span class="px-3 py-1 text-xs rounded-full
                                                                bg-indigo-50 text-indigo-700">
                                            {{ $req }}
                                        </span>
                                    @endforeach
                                </div>
                            </td>

                            <td class="px-5 py-4 text-right space-x-2">
                                <button wire:click="edit({{ $service->id }})" class="px-3 py-1.5 text-xs rounded-lg
                                            bg-blue-100 text-blue-700">
                                    Edit
                                </button>

                                <button wire:click="delete({{ $service->id }})" class="px-3 py-1.5 text-xs rounded-lg
                                            bg-red-100 text-red-600">
                                    Delete
                                </button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-10 text-center text-gray-500">
                                No services added yet
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>

    @if($showModal)
        <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-3">

            <div class="bg-white dark:bg-gray-800
                                w-full max-w-lg
                                rounded-md
                                max-h-[90vh]
                                flex flex-col">

                <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700">
                    <h2 class="text-lg font-semibold">
                        {{ $isEdit ? 'Edit Service' : 'Add Service' }}
                    </h2>
                </div>

                <div class="p-5 space-y-4 overflow-y-auto">
                    <div>
                        <label class="text-sm font-medium">Service Name</label>
                        <input wire:model.defer="name" type="text" class="w-full h-11 mt-1 px-3 rounded-md border">
                    </div>

                    <div>
                        <label class="text-sm font-medium">Service Image</label>

                        <input type="file"
                            wire:model="image"
                            accept="image/*"
                            class="w-full h-11 mt-1 px-3 rounded-md border">

                        <div wire:loading wire:target="image" class="text-xs text-gray-500 mt-1">
                            Uploading...
                        </div>

                        {{-- New image preview (Livewire temp) --}}
                        @if ($image)
                            <img src="{{ $existingImageUrl }}?tr=w-300,h-200,fo-auto" class="mt-3 h-24 rounded-md border object-cover">

                        @endif

                        {{-- Existing image preview (ImageKit) --}}
                        @if (!$image && $existingImageUrl)
                            <img src="{{ $existingImageUrl }}?tr=w-300,h-200,fo-auto" class="mt-3 h-24 rounded-md border object-cover">

                        @endif
                    </div>




                    <div>
                        <label class="text-sm font-medium">Requirements</label>

                        <div class="space-y-2 mt-2">
                            @foreach($requirements as $index => $req)
                                <div class="flex gap-2">
                                    <input type="text" wire:model="requirements.{{ $index }}" placeholder="Enter requirement"
                                        class="flex-1 h-11 px-3 rounded-md border">

                                    <button type="button" wire:click="removeRequirement({{ $index }})"
                                        class="w-11 h-11 rounded-md bg-red-100 text-red-600">
                                        ✕
                                    </button>
                                </div>
                            @endforeach
                        </div>

                        <button wire:click="addRequirement" class="mt-3 text-sm text-indigo-600 font-semibold">
                            + Add Requirement
                        </button>
                    </div>

                </div>

                <div class="px-5 py-4 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-2">
                    <button wire:click="$set('showModal', false)" class="px-4 py-2 rounded-md border">
                        Cancel
                    </button>

                    <button wire:click="save" class="px-4 py-2 rounded-md bg-indigo-600 text-white">
                        Save
                    </button>
                </div>

            </div>
        </div>
    @endif


</div>