...
<body class="h-full text-gray-900 antialiased" x-data="{ sidebarOpen: false }">
    ...
    <div x-data="{
        ...
        show = true;
        start();
    " x-show="show" x-transition:enter="transform ease-out duration-300 transition" x-transition:enter-start="translate-y-2 opacity-0 sm:translate-y-0 sm:translate-x-2" x-transition:enter-end="translate-y-0 opacity-100 sm:translate-x-0" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" x-cloak class="fixed z-50 top-4 right-4 w-[calc(100%-2rem)] sm:w-full sm:max-w-sm pointer-events-auto">
    ...
    <!-- MOBILE OVERLAY -->
    <div x-show="sidebarOpen" x-transition.opacity @click="sidebarOpen = false" class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm z-30 md:hidden" style="display: none;"></div>

    <div class="flex w-full h-full">

        <!-- ================= SIDEBAR ================= -->
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-gray-200 flex flex-col md:translate-x-0 transition-transform duration-300 ease-in-out">
        ...
        <div class="flex-1 flex flex-col md:ml-64 min-w-0 min-h-screen">
            ...
            <header class="shrink-0 sticky top-0 z-30 h-16 bg-white border-b border-gray-200 px-4 sm:px-6 flex items-center justify-between shadow-sm">
                ...
                <div class="flex items-center gap-4">
                    <button @click="sidebarOpen = true" class="md:hidden text-gray-500 hover:text-gray-700 transition-colors p-1 rounded-md hover:bg-gray-100">
